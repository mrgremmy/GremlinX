...
import { Address, AddressTypes, ChallengeSolution, IP2WSHAddress } from '../../node_modules/@btc-vision/transaction/build/index.js';
...
export declare abstract class AbstractRpcProvider {
    ...
    getCode(address: string | Address, onlyBytecode?: boolean): Promise<ContractData | Uint8Array>;
    getStorageAt(address: string | Address, rawPointer: bigint | string, proofs?: boolean, height?: BigNumberish): Promise<StoredValue>;
    call(to: string | Address, data: Uint8Array | string, from?: Address, height?: BigNumberish, simulatedTransaction?: ParsedSimulatedTransaction, accessList?: IAccessList): Promise<CallResult | ICallRequestError>;
    ...
    getPendingTransaction(hash: string): Promise<MempoolTransactionData<OPNetTransactionTypes> | null>;
    getLatestPendingTransactions(options?: {
        address?: string;
        limit?: number;
    }): Promise<MempoolTransactionData<OPNetTransactionTypes>[]>;
    getLatestPendingTransactionsByAddresses(options: {
        addresses: string[];
        limit?: number;
    }): Promise<MempoolTransactionData<OPNetTransactionTypes>[]>;
    protected abstract providerUrl(url: string): string;
    private _fetchPendingTransactions;
